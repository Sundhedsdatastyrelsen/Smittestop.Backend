...
namespace DIGNDB.App.SmitteStop.Testing.ServiceTest
{
    ...
    public class LogMessageValidatorTests
    {
        ...
        {
            LogMessageMobile log = new LogMessageMobile {reportedTime = invalidDate};
            var expectExceptionMsg = $"Could not parse reportedTime: {invalidDate}";
            LogMessageValidator validator = new LogMessageValidator();
            var exception = Assert.Throws<JsonException>(() => validator.ValidateLogMobileMessageReportedTime(log));
        ...
        {
            LogMessageMobile log = new LogMessageMobile {reportedTime = invalidDate};
            var expectExceptionMsg = $"Value of reportedTime is more than two years from: {invalidDate} (Parameter '{nameof(log.reportedTime)}')";
            LogMessageValidator validator = new LogMessageValidator();
            var exception = Assert.Throws<ArgumentException>(() => validator.ValidateLogMobileMessageReportedTime(log));
            Assert.That(exception.Message, Is.EqualTo(expectExceptionMsg));
        }
        ...
        public void ValidateLogMobileMessagePatterns_LogMessageHasInValidData_ShouldThrowError(LogMessageMobile log)
        {
            ...
            Assert.That(exception.Message.StartsWith(expectExceptionMsg));
        }
